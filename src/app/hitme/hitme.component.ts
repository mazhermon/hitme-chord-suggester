...
import "firebase/firestore"; // may not need this line?
...
@Component({
  ...
  animations: [
    ...
    trigger("popout", [
      ...
      transition(":leave", [
        animate(
          "200ms cubic-bezier(.58,-0.45,.67,.99)",
          style({
            opacity: 0,
            transform: "translateY(10px) scale(0.8)"
          })
...
export class HitmeComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    this.userChordsSubscription = this.store
    ...
    this.hitmeChordsSubscription = this.store
      .pipe(select(fromHitMe.getHitmeChords), takeUntil(this.destoryed$))
      .subscribe(hitMeChords => (this.hitmeChords = hitMeChords));
    ...
    //leaving quick demo in for future ref
    // let currentUserEmail = this._authService.getCurrentUserEmail();
    // debugger;
    // this.db
    //   .collection("maz@test.com")
    //   .valueChanges()
    //   .subscribe(data => {
    //     console.log("from current user", data);
    //   });
  }
